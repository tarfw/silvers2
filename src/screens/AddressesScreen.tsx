                ...
                <View className="gap-4 mt-4">
                    ...
                    {isLoading ? (
                        ...
                        addressHistory.map((addr) => (
                            <TouchableOpacity
                                key={addr.id}
                                className="p-4 rounded-2xl bg-silver-50 border border-silver-100 mb-3"
                                onPress={() => navigation.navigate('AddAddress', { editingAddress: addr })}
                                activeOpacity={0.7}
                            >
                                <View className="flex-row justify-between">
                                    <View className="flex-1 mr-4">
                                        <View className="flex-row items-center mb-1">
                                            <View className={`px-2 py-0.5 rounded-md mr-2 ${addr.type === 'individual' ? 'bg-blue-50' : 'bg-amber-50'}`}>
                                        ...
                                        <View className="flex-row flex-wrap gap-x-4 gap-y-1">
                                            {addr.gst && (
                                                <Text className="text-xs text-brand-secondary">GST: <Text className="font-medium text-black">{addr.gst}</Text></Text>
                                            )}
                                        </View>
                                    </View>
